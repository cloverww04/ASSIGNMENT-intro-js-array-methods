...
const toggleCart = (event) => {
  if (event.target.id.includes("fav-btn")) {
   ...
   const index = referenceList.findIndex(indexId => indexId.id === Number(id))
...
const search = (event) => {
  ...
  const searchResult = referenceList.filter(filteredText => 
    filteredText.title.toLowerCase().includes(eventLC) ||
    filteredText.author.toLowerCase().includes(eventLC) ||
    filteredText.description.toLowerCase().includes(eventLC)
  )
...
const buttonFilter = (event) => {
  ...
  if(event.target.id.includes('cartFilter')) {
    const inTheCart = referenceList.filter(item => item.inCart === true);
...
const cartTotal = () => {
  ...
  const total = cart.reduce((initValue, objValue) => initValue + objValue.price, 0);
